...
When a player get 13 points, everyone else gets one more turn before
...
while True:  # Loop until the user enters a number
    ...
    if response.isdecimal() and int(response) > 1:
        ...
        break
    print("Player enter a number larger than 1.")
...
# (!) Uncommon to let a player named 'Al' start with three points:
# playerScores['Al'] = 3
endGameWith = None
while True:  # Main game loop.
    ...
    while True:
        ...
        if (3 - len(hand)) > len(cup):
            print(
                "There aren't enough dice left in the cup to continue "
                + playerNames[turn]
                + "'s turn."
            )
            break
        ...
        for dice in hand:
            ...
            if dice == GOLD:
                if 1 <= roll <= 3:
                    rollResult.append(STAR_FACE)
                    stars += 1
                elif 4 <= roll <= 5:
                    rollResult.append(QUESTION_FACE)
                else:
                    rollResult.append(SKULL_FACE)
                    skulls += 1
            if dice == SILVER:
                if 1 <= roll <= 2:
                    rollResult.append(STAR_FACE)
        ...
        for lineNum in range(FACE_HEIGHT):
            for diceNum in range(3):
                print(rollResult[diceNum][lineNum] + " ", end="")
            print()
        ...
        if response.startswith("N"):
            ...
            if endGameWith == None and playerScores[playerNames[turn]] >= 13:
                ...
                print("Everyone will get one more turn!")
                print(("!" * 60) + "\n\n")
...
if len(winners) == 1:
    ...
    print("The winner are: " + ", ".join(winners))
