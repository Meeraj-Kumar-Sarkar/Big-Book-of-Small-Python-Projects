...
try:
    ...
    print(
        "This program requires the bext module, which you can install by following the instructions at"
    )
...
def main():
    ...
    bext.clear()
    print("Flooder")
    print("Do you want to play in colorblind? Y/N")
    response = input("> ")
    ...
    while True:
        ...
        if hasWon(gameBoard):
            ...
            displayBoard(gameBoard, displayMode)
            print("You have ran out of moves!")
            break
...
def changeTile(tileType, board, x, y, charToChange=None):
    ...
    elif y > 0 and board[(x, y - 1)] == charToChange:
        changeTile(tileType, board, x, y - 1, charToChange)
    elif x < BOARD_WIDTH - 1 and board[(x + 1, y)] == charToChange:
        changeTile(tileType, board, x + 1, y, charToChange)
    elif y < BOARD_HEIGHT - 1 and board[(x, y + 1)] == charToChange:
        changeTile(tileType, board, x, y + 1, charToChange)
